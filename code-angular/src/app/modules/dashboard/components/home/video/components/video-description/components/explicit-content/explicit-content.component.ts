...
@Component({
  selector: 'explicit-content',
  templateUrl: './explicit-content.component.html',
  styleUrls: ['./explicit-content.component.scss']
})
export class ExplicitContentComponent implements AfterViewInit {
    ...
    if(this.info){
      ...
      if(this.info.length > 0) {
        this.info.forEach((f : any) => {
          const time = this.convertTime(f.frame_time, 0, 0);
          this.objects.push(
            {
              entity : f.pornography,
